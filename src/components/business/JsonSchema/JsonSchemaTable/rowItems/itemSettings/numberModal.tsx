import React, { useEffect } from 'react';
import { useTranslation } from 'react-i18next';

import { Flex, Input, InputNumber, Select, Switch } from 'antd';

import { useSafeState } from 'ahooks';
import { isEmpty, isNumber, trim } from 'lodash';

const NumberModal = (props: any) => {
  const {
    value,
    onChange,
    isRequired,
    handleUpdateRequired,
    handleChangeAllowNull,
    empty,
    SelectTypeDom,
    ItemMockDom,
  } = props;
  const { t } = useTranslation();
  const [caseValue, setCaseValue] = useSafeState([]);
  useEffect(() => {
    setCaseValue(value.enum);
  }, [value.enum]);

  return (
    <>
      <div className="base-setting">
        <div className="item-line">{SelectTypeDom}</div>
        <Flex gap={20} className="item-all-line item-necessarily">
          <Flex align="center" gap={8}>
            {t('common.schema.advanced_settings.required')}
            <Switch
              size="small"
              onChange={() => handleUpdateRequired(isRequired)}
              value={isRequired}
            />
          </Flex>
          <Flex align="center" gap={8}>
            {t('common.schema.advanced_settings.null')}
            <Switch size="small" value={empty} onChange={handleChangeAllowNull} />
          </Flex>
        </Flex>
        {ItemMockDom && (
          <Flex gap={20}>
            <div className="item-line">
              <div className="case-title">example</div>
              <div className="case-value">{ItemMockDom}</div>
            </div>
          </Flex>
        )}
        <Flex vertical>
          <div className="item-line">
            <div className="case-title">{t('common.schema.advanced_settings.default_values')}</div>
            <div className="case-value">
              <InputNumber
                style={{ width: '100%' }}
                size="small"
                min={-Infinity}
                value={isNumber(value?.default) ? value?.default : 0}
                onChange={onChange.bind(null, 'default')}
              />
            </div>
          </div>
        </Flex>
        <Flex gap={20}>
          <div className="item-line">
            <div className="case-title">{t('common.schema.advanced_settings.min_value')}</div>
            <div className="case-value">
              <InputNumber
                size="small"
                style={{ width: '100%' }}
                min={0}
                value={isNumber(value?.minimum) ? value?.minimum : undefined}
                onChange={onChange.bind(null, 'minimum')}
              />
            </div>
          </div>
          <div className="item-line">
            <div className="case-title">{t('common.schema.advanced_settings.eq_min_value')}</div>
            <Switch
              size="small"
              checked={value?.exclusiveMinimum}
              onChange={onChange.bind(null, 'exclusiveMinimum')}
            />
          </div>
        </Flex>
        <Flex gap={20}>
          <div className="item-line">
            <div className="case-title">{t('common.schema.advanced_settings.max_value')}</div>
            <div className="case-value">
              <InputNumber
                size="small"
                min={0}
                style={{ width: '100%' }}
                value={isNumber(value?.maximum) ? value?.maximum : undefined}
                onChange={onChange.bind(null, 'maximum')}
              />
            </div>
          </div>
          <div className="item-line">
            <div className="case-title">{t('common.schema.advanced_settings.eq_max_value')}</div>
            <Switch
              size="small"
              checked={value?.exclusiveMaximum}
              onChange={onChange.bind(null, 'exclusiveMaximum')}
            />
          </div>
        </Flex>

        <div className="item-line">
          <div className="case-title">
            <span style={{ paddingRight: 5 }}>
              {t('common.schema.advanced_settings.enum_values')}
            </span>
            <Switch
              size="small"
              checked={value?.apipost_enable_enum === true ? true : false}
              onChange={onChange.bind(null, 'apipost_enable_enum', !value?.apipost_enable_enum)}
            />
          </div>
          <div className="case-value">
            <Select
              style={{ width: '100%' }}
              disabled={value?.apipost_enable_enum !== true}
              mode="tags"
              value={caseValue}
              onChange={(e) => {
                setCaseValue(e);
                const numberList = [];
                for (const valItem of e) {
                  const parsedValue = !isEmpty(trim(valItem)) ? Number(valItem) : null;
                  if (isNumber(parsedValue) && !isNaN(parsedValue)) {
                    numberList.push(parsedValue);
                  }
                }
                onChange('enum', numberList);
              }}
              options={caseValue?.map((e) => ({ label: e, value: e }))}
            />
          </div>
        </div>
        <div className="item-line">
          <div className="case-title">{t('common.schema.advanced_settings.remarks')}</div>
          <div className="case-value">
            <Input.TextArea
              size="small"
              autoSize={{ minRows: 3, maxRows: 3 }}
              value={value?.enumDesc}
              onChange={(e) => {
                onChange('enumDesc', e.target.value);
              }}
            />
          </div>
        </div>
      </div>
    </>
  );
};

export default NumberModal;
