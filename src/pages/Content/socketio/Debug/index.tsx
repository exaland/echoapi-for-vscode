...
import { SocketIoComponentType, SocketIoDetailsData } from '@/types/apis/socketio';
import { AnyObject, DirectionType } from '@/types/common';
import { genNewWsByUrlChange, genQueryByUrl } from '@/utils/apis';
...
const SocketIoDebug: FC<SocketIoComponentType> = ({ apisData, onApisDataChange }) => {
  ...
  const handleRequest = useMemoizedFn((data: SocketIoDetailsData) => {
  ...
  return (
    <>
      <ToolBar apisData={apisData as unknown as ApiDetailsData} onApisDataChange={(val) => {
        onApisDataChange(val as unknown as SocketIoDetailsData);
      }} />
      <WsDebugContainer>
          ...
          <Flex flex={1} justify="space-between" align="center">
            <div className="url-group">
              <UrlGroup
                ...
                type={APIS_TARGET_TYPE_ENUM.SOCKETIO}
        ...
        <ResizablePanels
          showDirectionBtn
          panelGroupProps={{
            autoSaveId: 'socketio_debug_container_resize_save_id',
          ...
          rightPanel={
            <SuspenseContent>
              <div style={{ padding: '0 16px', height: '100%' }}>
                <Response apiScreenDirection={apiScreenDirection} value={apisData} onChange={handleRequest} />
              </div>
            </SuspenseContent>
          }
